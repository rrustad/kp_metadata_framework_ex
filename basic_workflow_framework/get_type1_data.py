...
dbutils.widgets.text('source_table_path', 'kp_catalog.clarity_stg.table1')
source_table_path = dbutils.widgets.get('source_table_path')
dbutils.widgets.text('target_table_path', 'kp_catalog.clarity_enr.table1')
target_table_path = dbutils.widgets.get('target_table_path')
dbutils.widgets.text('primary_keys', 'tb1pk1')
...
dbutils.widgets.text('delete_table_path', 'kp_catalog.clarity_stg.table1_del')
delete_table_path = dbutils.widgets.get('delete_table_path')
dbutils.widgets.text('table_name', '')
table_name = dbutils.widgets.get('table_name')
dbutils.widgets.text('checkpoints_path', '')
...
if table_exists(source_table_path+'_updates'):
...
(
  df_src_rank
  .writeStream
  .option("checkpointLocation", checkpoints_path+source_table_path.split('.')[-1])
...
(
  delete_df
  .writeStream
  .option("checkpointLocation", checkpoints_path+delete_table_path.split('.')[-1])
  .trigger(availableNow=True)
  .toTable(source_table_path+'_updates')
  .awaitTermination()
)
...
spark.table(source_table_path+'_updates').display()
...
(
  ...
  .withColumn('rnk', f.dense_rank().over(
    Window.partitionBy(pks).orderBy(
...
df = spark.sql(f"""
MERGE INTO {target_table_path} TGT
...
spark.sql(f"update {source_catalog}.clarity_stg.table_status SET status_ready = 'N' WHERE source_table = '{table_name}'")
